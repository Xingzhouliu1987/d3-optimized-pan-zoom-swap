...
function fastZoomPan() {
      ...
      var widget = {}
  var dispatcher = d3.dispatch("recenter","preswap","postswap","postrecenter")

      var validations = {
        container : d3utils.isType.selection ,
        bbox : function(val) { return d3utils.isType.func(val) || (val.length == 2 && val[0].length == 2 & val[1].length ==2 && alltrue(val[0], d3utils.isType.number) && alltrue(val[1], d3utils.isType.number)) }
      }
  ...
  //   //node.clientWidth;
  // }
  function swap_visible(hidden_layer) {
      if(widget.svg().attr("data-layer-name")==hidden_layer || params.__hidden[hidden_layer] == null) return widget;
      dispatcher.call("preswap", widget, params._svg.attr("data-layer-name"), hidden_layer)
      params._wrapper.style("pointer-events","none");
      ...
      swap_out.transition()
              ...
              .style("opacity",0.0)
              .on("end",function(){
                     widget.zoom().on("zoom.check_resolution", check_resolution_after_zoom(params._svg , t))
                     swap_out.style("z-index",50).style("display","none")
                     swap_in.style("z-index",100)
                     widget.wrapper().call( widget.zoom().transform , t)
                     params._wrapper.style("pointer-events","all")
  ...
  widget.swap_visible = swap_visible;
      ...
      */
      function transformTo(d, scale) {
          scale = d3utils.isType.number(scale) ? scale : getTransform().k;
          if(d && d3utils.alltrue(d, d3utils.isType.number)) {
              return d3.zoomIdentity.translate(params.width / 2 - d[0] * scale , params.height / 2 - d[1] * scale).scale(scale)
          }
          return transformTo([params._svgdim.width / 2 , params._svgdim.height / 2], 1)
      }
  ...
  function setzoom(zoom,d3c) {
      ...
      params._zoom = zoom;
      widget.wrapper().call( zoom.on("zoom."+widget.id().replace("-","_"), zoomHandler( widget.svg , d3c ) ) )


      //widget.wrapper().call( widget.zoom().transform , transformTo(null, 1) )
             widget.zoom().on("zoom.check_resolution",
