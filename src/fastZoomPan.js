  ...
  */
  function transformTo(d, scale, bounded) {
      ...
      if(d && d3utils.alltrue(d, d3utils.isType.number)) {
          ...
          //require transforms to be bounded
          scale = Math.min( scalextent[1] , Math.max(scale, scalextent[0]) )
          var xt = params.width/2-Math.max(Math.min(params._svgdim.width * scale - params.width / 2, scale*d[0]),params.width/2),
yt = params.height/2-Math.max(Math.min(params._svgdim.height * scale - params.height / 2, scale*d[1]),params.height/2);
  ...
  function transform(center, scale, notransition, enforcebds) {
       return widget.wrapper()
             .transition()
             .duration(params.tDuration)
            // .ease(params.tEase)
             .call(widget.zoom().transform, widget.transformTo(center,scale,enforcebds))
  }
